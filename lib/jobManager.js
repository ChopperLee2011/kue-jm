...
class JobManager extends EventEmitter {
          ...
          job
            .on('complete', (result) => {
              ...
              if (resultTTL < 0)
                ...
                // todo: implement it with notification service after job complete.
                if (opt.notificationSerive)
                  opt.notificationSerive(opt.payload);
                jobDb.setex(key, resultTTL, result);
              }
            })
            .on('failed', (errorMsg) => {
              throw new Error(`job failed, error message is: ${errorMsg}`);
            })
            .save((err) => {
              if (err) {
                jobDb.del(key);
  ...
  run(type, concurrency) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.job.collections.get(type) === undefined)
        return reject(new Error('wrong job type'));
      this.task._queue.process(type, concurrency, (job, done) => {
        ...
        this.sortTasks('chain', sid, tasks)
          .then(() => {
            // todo: blocking queue processing have bad pref, should change it.
            return series.execute(this, sid, tasks);
          })
          .then((res) => {
   ...
   * @returns {Promise}
   */
  sortTasks(type, key, tasks) {
    const multi = this.job._db.multi();
    debug('sortTasks type: %s \t key: %s \t tasks: %o', type, key, tasks);
    return new Promise((resolve, reject) => {
      if (type === 'chain') {
        tasks.forEach((task, i) => {
          tasks[i].idx = i;
          multi.hmset(`${key}:${i}`, util.serialize(task));
        });
        multi.exec((err) => {
          if (err)
            return reject(err);
